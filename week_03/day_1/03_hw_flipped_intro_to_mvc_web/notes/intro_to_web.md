...
The client may also request changes to the resource, or create new resources (this is where POST comes in).
...
For these examples, we can clearly see what 'resource' we are requesting. Depending on how we ask for it, we might get something different back. For example, we may ask for all the videos, one video, add a video or even delete a video. 

The request-response part of the protocol means that the server can only send data to the client in response to a request from the client (i.e. synchronously). The server can not arbitrarily send data to the client unless a request is made.
...
This week we will look at a popular framework that is written in Python: Flask.  There are many others.  JavaEE, ASP.net(c#), Express(JS), Symphony(PHP).  
